...
def one_round_exp(env, lat_act, lon_act, base_speed, stable_time, collect_time_window_width, 
    ...
    for _ in range(int(stable_time * WAYMO_SAMPLING_FREQ)):
        global_vel, heading = get_current_pose(env)
        lon_speed = global_vel [0]*np.cos(heading) + global_vel[1]*np.sin(heading)
        
        action_lon, error_sum, last_error= pid_controller(base_speed,lon_speed, error_sum, last_error, 1/WAYMO_SAMPLING_FREQ)
    ...
    if plot_speed_before_stablization:
        ...
        plt.plot(np.arange(len(speeds))/WAYMO_SAMPLING_FREQ, speeds, label='speed')
        plt.legend()
        plt.xlabel('Time')
        plt.ylabel('speed')
    ...
    vels = []
    headings = []
    for _ in range(int(collect_time_window_width * WAYMO_SAMPLING_FREQ)):
    ...
    ts = np.arange(int(collect_time_window_width * WAYMO_SAMPLING_FREQ))/WAYMO_SAMPLING_FREQ
    ...
    if fit_visualization:
        ...
        plt.title('Time vs. Velocity(acc lat = '+str(lat_acc)+' m/(s^2))')
        plt.show()
    ...
    for i in range(num_dp_per_dim): # action 0
        for j in range(num_dp_per_dim): # action 1
            for base_speed in speeds:
                lat_acc, lon_acc, lat_sse, lon_sse = one_round_exp(env, lats[i], lons[j], base_speed, stable_time, collect_window_width)

                # log data: save as vars(lat action input, lon action input, base speed) + lat_acc + lon_acc
                print("lat action input, lon action input, base speed) + lat_acc + lon_acc = ",lats[i], lons[j], base_speed, lat_acc, lon_acc, lat_sse, lon_sse)
